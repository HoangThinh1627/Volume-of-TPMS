...
cfit = polyval(ptest,plotp(2,:));
cfit1 = polyval(Pcoef,plotp(2,:));
plot(plotp(2,:), cfit1,'LineWidth', 2);
plot(plotp(2,:), cfit,'LineWidth', 2,'Color', 'blue');
...
SStot1 = sum((plotp(1,:) - mean(cfit)).^2); % Total sum of squares
SSres1 = sum((plotp(1,:) - cfit).^2); % Residual sum of squares
R_squared1 = 1 - (SSres1 / SStot1); % R-squared value


% Add R^2 annotation
annotation('textbox', [0.15, 0.8, 0.2, 0.1], 'String', ...
    {['R^2 for data = ', num2str(R_squared)], ['R^2 for equations = ', num2str(R_squared1)]}, ...
